namespace KeyTutor {
    public class Widgets.CustomMButton : Gtk.Box {
        private class Item : Gtk.ToggleButton {
            public int index { get; construct; }
            public Item (int index, string? tt_text = null) {
                Object (index: index);
                can_focus = false;
        ...
        public int append_text (string text, string? tooltip = null) {
        ...
        public int append (Gtk.Widget w, string? tooltip = null) {
            int index;
            for (index = item_map.size; item_map.has_key (index); index++);
            assert (item_map[index] == null);

            var item = new Item (index, tooltip);
